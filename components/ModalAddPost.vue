<template>
  <div class="bg-black bg-opacity-25 h-screen fixed top-0 right-0 left-0 z-40 flex" :class="method.modalStat ? 'visible' : 'invisible'">
    <div :class="method.modalStat ? 'bottom-0' : '-bottom-full'" class="left-0 z-40 transition-all duration-500 ease-in-out h-[90vh] absolute right-0 border rounded-t-[50px] bg-white w-full justify-center py-10 flex">
      <button @click="method.modalFnc()" class="right-10 absolute top-9 z-50 border-2 rounded-full p-2">
        <PhosphorIconX />
      </button>
