<template>
  <div class="bg-black bg-opacity-25 h-screen fixed top-0 right-0 left-0 z-40" :class="method.modalStat ? 'flex' : 'hidden'">
    <button @click="method.modalFnc()" class="right-10 absolute top-24 z-50 border-2 rounded-full p-2">
      <PhosphorIconX />
    </button>
    <form @submit.prevent="submit" class="bottom-0 left-0 z-40 absolute right-0 h-[90vh] border rounded-t-[50px] flex-col items-center bg-white w-full justify-center py-10 flex">
